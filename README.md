...
- Python 3
- Python-пакеты для запуска: `pystray`, `pillow`
...
- Если в Windows 11 не появляется toast-уведомление, проверьте режим «Не беспокоить» (Focus Assist) в настройках уведомлений.
...
- Windows-режим использует `pystray` для трея и встроенные Toast-уведомления Windows (через `powershell.exe`); появление баннера зависит от системных настроек уведомлений.
